...
describe('parseMojos', () => {
  ...
  describe('BigNumber safety', () => {
    it('rejects non-finite values', () => {
      ...
      expect(() => parseMojos(new BigNumber(Number.POSITIVE_INFINITY))).toThrow(
        'Mojos must be finite, got: Infinity',
      );
      expect(() => parseMojos(new BigNumber(Number.NEGATIVE_INFINITY))).toThrow(
        'Mojos must be finite, got: -Infinity',
      );
    });
