...
test('Verify that an NFT can be Transfer to another account', async () => {
  // Pre-requisites
  const receive_nft_wallet = 'txch15zuw5285d075w8ge99sz23tdcf9kyq2pzez9x3x82z22l5u8g68qs5gec8';
  const send_wallet = 'txch1z23pn6l698hxygxrrdz0sz3j87dnuylp52d53h96tl4g5p09qtes5r5r2f';
  ...
  await page.getByTestId('DashboardSideBar-nfts').click();
  await page.waitForSelector('text=Bobble #3', { state: 'visible', timeout: 300_000 });
  await page.getByRole('button', { name: 'Bobble #3' }).getByRole('button').click();
  await page.getByRole('menuitem', { name: 'Transfer NFT' }).click();
